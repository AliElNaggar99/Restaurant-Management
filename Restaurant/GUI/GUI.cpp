...
GUI::GUI()
{
	...
	DrawingColors[TYPE_NORMAL] =  RED;	//normal-order color
	DrawingColors[TYPE_VEGAN] = DARKBLUE;		//vegan-order color
...
void GUI::PrintMessageML(string *msg , int Lines) const	//Prints a message on status bar
{
	...
	for (int i = 0; i < Lines; i++)
	{
		...
		pWind->DrawString(10, WindHeight - (int)((5-i)*StatusBarHeight / (7)+20), msg[i]); // You may need to change these coordinates later 
																		  // to be able to write multi-line
	}
}
